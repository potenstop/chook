...
function NotBank(target, propertyKey, paramIndex) {
...
exports.NotBank = NotBank;
function exec(target, propertyKey, paramIndex, options) {
    if (options.value) {
        if (StringUtil_1.StringUtil.isEmpty(options.message)) {
            options.message = "is bank";
        }
        if (JSHelperUtil_1.JSHelperUtil.isNotNull(paramIndex)) {
            const metadataKeys = Reflect.getOwnMetadata(MetaConstant_1.MetaConstant.VALID_NOTBANK, target.constructor, propertyKey) || [];
            ...
            Reflect.defineMetadata(MetaConstant_1.MetaConstant.VALID_NOTBANK, metadataKeys, target.constructor, propertyKey);
        }
        else {
            const metadataKeys = Reflect.getOwnMetadata(MetaConstant_1.MetaConstant.VALID_NOTBANK, target.constructor) || new Map();
            const validMeta = new ValidMeta_1.ValidMeta();
            validMeta.options = options;
            metadataKeys.set(propertyKey, validMeta);
            Reflect.defineMetadata(MetaConstant_1.MetaConstant.VALID_NOTBANK, metadataKeys, target.constructor);
        }
    }
}
//# sourceMappingURL=NotBank.js.map