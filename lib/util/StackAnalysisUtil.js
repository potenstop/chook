...
const StackTypeModeEnum_1 = require("../enums/StackTypeModeEnum");
const StackTypePathTypeEnum_1 = require("../enums/StackTypePathTypeEnum");
const StackType_1 = require("../model/StackType");
/**
 *
 * 功能描述: 堆栈信息解析工具类
...
class StackAnalysisUtil {
     ...
     */
    static parseStack(stack, pathType) {
        const list = [];
        ...
        for (let current of stackList) {
            ...
            const stackType = new StackType_1.default();
            stackType.className = "-";
            stackType.methodName = "-";
            stackType.mode = StackTypeModeEnum_1.StackTypeModeEnum.OBJECT;
            stackType.file = "";
            stackType.line = 0;
            stackType.row = 0;
            stackType.pathType = StackTypePathTypeEnum_1.StackTypePathTypeEnum.ERROR;
            current = current.replace(trimSurplusRe, "");
            let fileLineRow;
            if (current[current.length - 1] !== ")") {
                stackType.mode = StackTypeModeEnum_1.StackTypeModeEnum.NONE;
                fileLineRow = current;
            }
            else {
                ...
                if (a.length === 1) {
                    if (/^new\s/.test(a[0])) {
                        stackType.mode = StackTypeModeEnum_1.StackTypeModeEnum.CLASS;
                        stackType.className = a[0].substring(4, a[0].length);
                        stackType.methodName = "constructor";
                    }
                    else {
                        stackType.mode = StackTypeModeEnum_1.StackTypeModeEnum.FUNCTION;
                        stackType.methodName = a[0];
                    }
                }
                else if (a.length === 2) {
                    if (o === "Object.<anonymous>") {
                        stackType.mode = StackTypeModeEnum_1.StackTypeModeEnum.OBJECT_ANONYMOUS;
                        stackType.className = "Object";
                    }
                    else if (a[0] === "Object") {
                        stackType.mode = StackTypeModeEnum_1.StackTypeModeEnum.OBJECT_FUNCTION;
                        stackType.className = "Object";
                        stackType.methodName = a[1];
                    }
                    else if (a[0] === "Function") {
                        stackType.mode = StackTypeModeEnum_1.StackTypeModeEnum.FUNCTION_FUNCTION;
                        stackType.className = "Function";
                        stackType.methodName = a[1];
                    }
                    else {
                        stackType.mode = StackTypeModeEnum_1.StackTypeModeEnum.CLASS_FUNCTION;
                        stackType.className = a[0];
                        stackType.methodName = a[1].replace(asFuncRe, "");
                    }
                }
                else if (a.length === 3) {
                    stackType.mode = a[0] === "Object" ? StackTypeModeEnum_1.StackTypeModeEnum.OBJECT_DOUBLE_FUNCTION : StackTypeModeEnum_1.StackTypeModeEnum.FUNCTION_PROTOTYPE_FUNCTION;
            ...
            if (match && match.length === 3) {
                ...
                stackType.pathType = path.isAbsolute(stackType.file) ? StackTypePathTypeEnum_1.StackTypePathTypeEnum.LOCAL : StackTypePathTypeEnum_1.StackTypePathTypeEnum.SYSTEM;
            }
            if (pathType === StackTypePathTypeEnum_1.StackTypePathTypeEnum.ALL || stackType.pathType === pathType) {
                list.push(stackType);
            }
        }
        return list;
    }
    static parseStackAll(stack) {
        return StackAnalysisUtil.parseStack(stack, StackTypePathTypeEnum_1.StackTypePathTypeEnum.ALL);
    }
}
exports.StackAnalysisUtil = StackAnalysisUtil;
//# sourceMappingURL=StackAnalysisUtil.js.map