...
class JsonProtocol {
     ...
     * @return
     */
    static routerToBean(value, genericsProperty) {
        const genRoot = genericsProperty.get(CommonConstant_1.CommonConstant.GENERICS_ROOT);
        if (JSHelperUtil_1.JSHelperUtil.isNullOrUndefined(value)) {
            return null;
        }
        else if (JSHelperUtil_1.JSHelperUtil.isBaseObject(value) && JSHelperUtil_1.JSHelperUtil.isBaseType(genRoot)) {
            // 基础类型的泛型 则直接赋值
            if (genRoot === Number) {
                return Number(value);
            }
        ...
        else if ((typeof value === "object" || typeof value === "string") && (JSHelperUtil_1.JSHelperUtil.isClassType(genRoot) || Array === genRoot)) {
            let jsonParse;
